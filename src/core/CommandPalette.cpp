...
QVector<CommandSpec> CommandPalette::builtins() {
  return {
      ...
      {QStringLiteral("sql"), QStringLiteral("SQL file workbench"), true},
      {QStringLiteral("ask"),
       QStringLiteral("Find files with the default agent"), true},
      {QStringLiteral("semantic"),
       QStringLiteral("Find images by visual meaning"), true},
      {QStringLiteral("see"), QStringLiteral("Find images by visual meaning"),
       true},
      {QStringLiteral("settings"), QStringLiteral("Indexer settings"), true},
      {QStringLiteral("flow"), QStringLiteral("Omaflow automations"), true},
...
const std::initializer_list<HelpSection> kHelp = {
    {"Navigate",
     {{"Arrow keys", "move through the rendered listing"},
      {"Enter", "open selected item"},
      {"Backspace", "up a folder"},
      {"Alt+Left / Right", "back / forward"},
      {"type", "filter this folder"},
      {"Esc", "clear filter and restore position"}}},
    {"Look & Peek",
     {{"Space", "toggle Look"},
      {"Shift+Space", "open full Peek"},
      {"Enter", "open / send"},
      {"A / D", "step index / file"},
    ...
    {"Do layer",
     {{"Ctrl+Enter", "open (also right-click)"},
      ...
      {"Esc / Q", "leave"}}},
    {"Find & fields",
      {{"type or /", "filter this folder"},
      {":", "command"},
      {"Ctrl+L", "jump to path"},
      {"Ctrl+K", "filter field"},
      {"? name", "name search (fd)"},
      {"?? text", "content search (rg)"},
      {":see blue images", "semantic image search"}}},
    {"Files",
    ...
    {"Commands",
      ...
      {":sql", "query here / tree / selection"},
      {":ask", "find files with Omarchy's agent"},
      {":see <idea>", "find images by visual meaning"},
      {":settings / Ctrl+,", "indexer settings"},
      {":flow", "inspect Omaflow automations"},
      {":help ? F1", "this help"}}},
